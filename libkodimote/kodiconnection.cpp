...
void KodiConnectionPrivate::versionReceived(const QVariantMap &rsp)
{
    ...
    QVariantMap version = rsp.value("result").toMap().value("version").toMap();
    m_kodiVersionMajor = version.value("major").toInt();
    m_kodiVersionMinor = version.value("minor").toInt();
    if(m_kodiVersionMajor < 6 || (m_kodiVersionMajor == 6 && m_kodiVersionMinor < 21)) {
        qDebug() << "WARNING! XBMC/Kodi is too old or version field not valid! Some features might not work";
        m_connectionError = tr("This version of Kodimote is designed to work with Kodi Helix (v14.0). It seems you have connected to an older version of XBMC/Kodi. Please upgrade Kodi in order to use Kodimote.");
    } else {
        m_connected = true;
